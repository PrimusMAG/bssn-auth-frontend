...
export class KonteksDetailComponent implements OnInit {
  ...
  appetiteOptions = ['LOW', 'MEDIUM', 'HIGH'];
  ...
  deleteRC(c: RiskCategory): void {
    const ok = confirm(`Hapus kategori "${c.name}"?`);
    if (!ok) return;
    ...
    this.http
      ...
      .subscribe({
        ...
        error: (e) => {
          this.loading = false;
          alert(e?.error?.errors || e?.error?.message || 'Gagal hapus kategori.');
  ...
  deleteLK(l: LikelihoodScale): void {
    const ok = confirm(`Hapus likelihood "${l.label}" (level ${l.level})?`);
    if (!ok) return;
    ...
    this.http
      ...
      .subscribe({
        ...
        error: (e) => {
          this.loading = false;
          alert(e?.error?.errors || e?.error?.message || 'Gagal hapus likelihood.');
